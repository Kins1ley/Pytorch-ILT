...
class DumpKernel(object):
    ...
    def read_scales(self):
        ...
        temp = torch.tensor(scales, dtype=torch.float32, device=device)
        # print(temp)
        return temp
    ...
    def read_kernels(self):
        ...
        for i in range(self.knum):
            ...
            for j in range(5, len(data)//4-1, 2):
                ...
                kernel.append(torch.tensor([real_values[0] + 1j*imag_values[0]], dtype=torch.complex64, device=device))
            kernels.append(kernel)
        kernels = torch.tensor(kernels, dtype=torch.complex64, device=device)
    ...
    def combo_kernel(self):
        ...
        self.scales = torch.tensor([1], dtype=torch.float32).to(device)
