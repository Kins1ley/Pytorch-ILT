...
Design::Design(const char *layoutFile)
...
void Design::parse(const char *layoutFile)
{
#ifndef _DEBUG
...
void Design::addRect(int llx, int lly, int urx, int ury)
{
...
void Design::testParser()
{
    for (vector<Rect *>::iterator it = m_rects.begin(); it != m_rects.end(); ++it)
        m_maskRects.push_back(*it);
}

void Design::writeGlp(const char *outputFile)
{
