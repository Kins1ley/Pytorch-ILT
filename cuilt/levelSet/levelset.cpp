...
void OPC::stochasticGD()
{
    ...
    do
    {
        ++numIteration;
        imesg("============================\n");
        imesg("Gradient search iteration %d......\n", numIteration);
...
#ifdef _DEBUG
        dmesg("Objective value before updating: %f\n", m_objConvergence[numIteration - 1]);
        dmesg("EPE count: %d\n", m_epeConvergence[numIteration - 1]);
        ...
        std::cout << "maxg: " << maxg << ", ming: " << ming << std::endl;
#endif
...
//writeMatrix("imageInner" + ss.str(), m_image[1]);
    ...
    dmesg("Final objective value: %f\n", m_objConvergence[numIteration]);
...
void OPC::calculateGradient_sig_pvband8(int numIteration)
{
    ...
    dmesg("pvband before updating = %d\n", m_pvband[numIteration - 1]);
#endif
...
void OPC::determineStepSize_ta(int numIteration)
{
    ...
    else if (numIteration > NUM_ITER_FIXED_STEP)
    {
        ...
        dmesg("%d gradient updated to opposite direction\n", count);
    }
    else
    {
...
void OPC::determineStepSize_backtrack(int numIteration)
{
    ...
    else
    {
        ...
        dmesg("%d stepsize jumpped; %d stepsize reduced\n", countJump, countReduce);
    }
}
...
void OPC::findKeepOutRegion()
{
    ...
    dmesg("keepout region: %d-%d\n", keepOutStart, keepOutEnd);
...
void OPC::restoreBestResult(float curObj)
{
    if (curObj > m_minObjValue)
    {
        dmesg("Restore previous best solution\n");
