...
EpeChecker::EpeChecker()
    : m_valid(false), m_numEPEIn(0), m_numEPEOut(0)
{
    m_bimage = new char[EPE_TILE_Y * EPE_TILE_Y];
}
...
void EpeChecker::setDesign(Design &design)
{
    m_valid = true;
...
void EpeChecker::setDesign(Design &design, float *targetImage)
{
    setDesign(design);
    m_targetImage = targetImage;
}

void EpeChecker::clear()
{
...
int EpeChecker::run(const vector<EpeSample> *samples, const float *image, const float *target)
{
    if (!m_valid)
...
//return number of total EPE violations
int EpeChecker::run(const float *image)
{
    if (!m_valid)
        emesg("must set design before checking EPE\n");
...
//obtain binary mask
void EpeChecker::binaryImage(const float *image)
{
    memset(m_bimage, 0, sizeof(char) * EPE_TILE_X * EPE_TILE_Y);

    int index;
    for (int y = EPE_OFFSET_Y; y < EPE_TILE_END_Y; ++y)
        for (int x = EPE_OFFSET_X; x < EPE_TILE_END_X; ++x)
        {
            index = getIndex(x, y);
            if (image[index] >= TARGET_INTENSITY)
                m_bimage[index] = 1;
        }
...
//direction points to internal of the pattern
void EpeChecker::check(int x, int y, direction_t direction)
{
    //apply offset
    x = x + EPE_OFFSET_X;
    y = y + EPE_OFFSET_Y;
...
void EpeChecker::setEPESafeRegion(float *matrix, int constraint)
{
    if (!m_valid)
        emesg("must set design before checking EPE\n");
...
void EpeChecker::setEPEBoundary(float *matrix, int constraint)
{
    if (!m_valid)
        emesg("must set design before checking EPE\n");
...
void EpeSample::setPosition(kiss_fft_cpx *from, kiss_fft_cpx *to) const
{
    int index;
...
void EpeSample::resetPosition(kiss_fft_cpx *matrix) const
{
    int index;
    if (m_orient == EpeChecker::HORIZONTAL)
    {
