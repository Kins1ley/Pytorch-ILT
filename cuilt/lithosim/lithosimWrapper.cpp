...
LithosimWrapper::LithosimWrapper()
{
    ...
    m_mask_1 = new unsigned char[OPC_TILE_SIZE];
    m_mask_2 = new unsigned char[OPC_TILE_SIZE];
    for (int i = 0; i < NUM_PROCESS_CORNERS; ++i)
    {
        m_image[i] = new float[OPC_TILE_SIZE];
        m_subImage_1[i] = new float[OPC_TILE_SIZE];
        m_subImage_2[i] = new float[OPC_TILE_SIZE];
    }
    initSimulate();
}
...
LithosimWrapper::~LithosimWrapper()
{
    ...
    {
        delete m_image[i];
        delete m_subImage_1[i];
        delete m_subImage_2[i];
    }
}
...
void LithosimWrapper::writeMaskPng(char *fileName, float *mask, int xSize, int ySize)
{
    int pixel;
    //  for (int i = 0; i < OPC_TILE_SIZE; ++i)
    //  {
    //    if (mask[0][i] >= MASK_PRINTABLE_THRESHOLD)
    //      pixel = 1;
    //    else
    //      pixel = 0;
    //    m_mask[0][i] = pixel * OPC_PIXEL_VALUE;
    //  }
    ...
    write_mask_file(m_mask, fileName, xSize, ySize);
    //write_mask_file(m_mask[1], fileName_2, xSize, ySize);
}
void LithosimWrapper::writeImagePng(char *fileName, float *image, float dose,
                                    int xSize, int ySize)
{
    write_image_file(image, fileName, xSize, ySize, dose);
}

void LithosimWrapper::writeImageEpePng(char *fileName, float *image, float dose,
                                       const char *inLayoutFile, int xSize, int ySize)
{
    if (inLayoutFile)
...
void LithosimWrapper::simulateImage(float *mask, unsigned char *image,
...
void LithosimWrapper::simulateImageOpt(float *mask, float *image,
                                       kernel_t kernelType, float dose, int kernelLevel)
{
...
void LithosimWrapper::convolveKernel(float *mask, kiss_fft_cpx *result,
                                     kernel_t kernelType, float dose, int kernelLevel)
{
...
void LithosimWrapper::convolveKernel(kiss_fft_cpx *matrix, kiss_fft_cpx *result,
                                     kernel_t kernelType, float dose, int kernelLevel)
{
...
int LithosimWrapper::calculatePvband(float *mask)
{
    //outer: focus, maxdose
...
int LithosimWrapper::calculatePvband(float *inner, float *outer)
{
    return getPvband(inner, outer);
}

void LithosimWrapper::writePvband(char *fileName)
{
    pvbandsim::writePvband(fileName, m_image[0], m_image[1]);
}

void LithosimWrapper::writePvband(char *fileName, float *inner, float *outer)
{
