...
#include "lithosim/pvbandsim/glpParse.h"
...
int er_none(int (*er)(char *l, int x, int y, int w, int h), char *l, int x1, int y1, int x2, int y2, int w)
{
    ...
    if (x1 == x2) /* vertical */
    {
        if (y1 < y2)
        {
            er(l, x1 - w / 2, y1, w, y2 - y1);
        }
        else
        {
            er(l, x1 - w / 2, y2, w, y1 - y2);
        }
    }
    else if (y1 == y2) /* horizontal */
    {
        if (x1 < x2)
        {
            er(l, x1, y1 - w / 2, x2 - x1, w);
        }
        else
        {
            er(l, x2, y1 - w / 2, x1 - x2, w);
        }
    }
    else
    {
        fprintf(stderr, "WARNING er_none: degenerate line with a diagonal on %s %d,%d -> %d,%d\n", l, x1, y1, x2, y2);
        ret = 0;
    }
    return ret;
}
...
int er_ext1(int (*er)(char *l, int x, int y, int w, int h), char *l, int x1, int y1, int x2, int y2, int w)
{
    ...
    {
        fprintf(stderr, "WARNING er_ext1: degenerate line with a diagonal on %s %d,%d -> %d,%d\n", l, x1, y1, x2, y2);
        ret = 0;
    }
    return ret;
}
...
int er_ext2(int (*er)(char *l, int x, int y, int w, int h), char *l, int x1, int y1, int x2, int y2, int w)
{
    ...
    {
        fprintf(stderr, "WARNING er_ext2: degenerate line with a diagonal on %s %d,%d -> %d,%d\n", l, x1, y1, x2, y2);
        ret = 0;
    }
    return ret;
}
...
int er_both(int (*er)(char *l, int x, int y, int w, int h), char *l, int x1, int y1, int x2, int y2, int w)
{
    ...
    {
        fprintf(stderr, "WARNING er_both: degenerate line with a diagonal on %s %d,%d -> %d,%d\n", l, x1, y1, x2, y2);
        ret = 0;
    }
    return ret;
}
...
int glp_el(int (*er)(char *l, int x, int y, int w, int h), char *l, int w, int n, int *xy)
{
    ...
    {
        if (er_none(er, l, x1, y1, x2, y2, w) == 0)
            line_err_prt(l, w, n, xy);
    }
    else /* multiple line segments */
    {
        /* first segement */
        if (er_ext2(er, l, x1, y1, x2, y2, w) == 0)
            line_err_prt(l, w, n, xy);
        /* middle segments */
        x1 = x2;
        y1 = y2;
        for (i = 2; i < n - 1; i++)
        {
            x2 = xy[k++];
            y2 = xy[k++];
            if (er_both(er, l, x1, y1, x2, y2, w) == 0)
                line_err_prt(l, w, n, xy);
            x1 = x2;
            y1 = y2;
        }
        ...
        x2 = xy[k++];
        y2 = xy[k++];
        if (er_ext1(er, l, x1, y1, x2, y2, w) == 0)
            line_err_prt(l, w, n, xy);
    }
...
int glpParseFileRect(const char *fname, int (*er)(char *l, int x, int y, int w, int h))
{
    glpParseFile(fname, er, glp_ep, glp_el, glp_ea);
    return 1;
}

int glpParseFile(const char *fname,
...
)
{
    ...
    // the parser
    return (KWD("BEGIN") && maybe(many(((KWD("EQUIV") && NAT(ignore) && NAT(micron_scale) && assign(micron_scale /= 1000) && KWD("MICRON") && KWD("+X,+Y")) || (KWD("HEADER") && ID(str) && STR(str) && maybe(many(KWD("+") && STR(str)))) || (KWD("LEVEL") && ID(str) && fprintf(stderr, "Found level %s\n", str)) || (KWD("CNAME") && ID(str)) || (KWD("ATTR") && ID(str) && STR(str)) || (KWD("NOOP"))))) && KWD("CELL") && ID(str) && KWD("PRIME")
            /* && printf("Found cell %s\n", str) */
            && maybe(many((KWD("ATTR") && ID(str) && ((KWD("X") && STR(str)) || STR(str))) || (KWD("RECT") && KWD("N") && ID(lyr) && INT(x) && INT(y) && INT(w) && INT(h)
                                                                                               //			      && printf("Rect %s\n",str)
                                                                                               && (*er)(lyr, x / micron_scale, y / micron_scale, w / micron_scale, h / micron_scale)) ||
                          (KWD("PGON") && ID(str) && ID(lyr) && assign(n = 0) && many(INT(x) && maybe(KWD("+")) && INT(y) && maybe(KWD("+")) && assign(xy[n++] = x / micron_scale) && assign(xy[n++] = y / micron_scale))
                           //			      && printf("Pgon %s\n", str)
                           && (*ep)(er, lyr, n / 2, xy)) ||
                          (KWD("LINE") && KWD("N") && (KWD("U") || KWD("E")) && ID(lyr) && assign(n = 0) && INT(w) && many(INT(x) && maybe(KWD("+")) && INT(y) && maybe(KWD("+")) && assign(xy[n++] = x / micron_scale) && assign(xy[n++] = y / micron_scale)) && (*el)(er, lyr, w / micron_scale, n / 2, xy)) || (KWD("ALPHA") && KWD("N") && ID(lyr) && INT(x) && INT(y) && INT(w) && INT(h) && INT(n) && STR(str) && (*ea)(lyr, x / micron_scale, y / micron_scale, w / micron_scale, h / micron_scale, str)))) &&
            KWD("ENDMSG") && END);
}
