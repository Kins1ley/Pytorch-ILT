...
#include "lithosim/pvbandsim/kiss_fftnd.h"
#include "lithosim/pvbandsim/kernels.h"
#include "lithosim/pvbandsim/layoutrw.h"
#include "lithosim/pvbandsim/pvbandsim.h"
//#include "omp.h"
...
#include "lithosim/pvbandsim/write_png.h"

// add for gpu
#if WITH_GPU
    #include "lithosim/pvbandsim_gpu/cufftShift.h"
    #include "lithosim/pvbandsim_gpu/my_shift_cuda.h"
    #include "lithosim/pvbandsim_gpu/kiss_fftnd_cuda.h"
#endif
...
int calculate_pvband_area(float *inner, float *outer, int sizex, int sizey)
{
    ...
    for (j = 0; j < sizey; j++)
    {
        ...
        {
            index = j * sizex + i;
            if (outer[index] >= TARGET_INTENSITY && inner[index] < TARGET_INTENSITY)
                pvb_area++;
        }
    }
    return pvb_area;
...
//  }
//}
void myShift_cpu(kiss_fft_cpx *in, kiss_fft_cpx *out)
{
    int i, j;
    int x, y;
    int imxh = imx / 2;
...
void myShift(kiss_fft_cpx *in, kiss_fft_cpx *out)
{
    if(USE_GPU){
        #if WITH_GPU
            my_shift_cuda(in, out);
        #else
            fprintf(stderr, "*Message*: NO CUDA compilation, downgrade to cpu\n");
            myShift_cpu(in, out);
        #endif
    }else{
        myShift_cpu(in, out);
...
void myfft(const kiss_fft_cpx *fin, kiss_fft_cpx *fout, int isinverse){
    if(USE_GPU){
        #if WITH_GPU
            kiss_fftnd_cuda(fin, fout, isinverse);
        #else
            fprintf(stderr, "*Message*: NO CUDA compilation, downgrade to cpu\n");
            if(isinverse == FFT_FORWORD){
                kiss_fftnd(fwdplan, fin, fout);
            }else{
                kiss_fftnd(bwdplan, fin, fout);
            }
        #endif
    }else{
        if(isinverse == FFT_FORWORD){
...
void myThreshhold(float *img, unsigned char *bimg)
{
    ...
    for (i = 0; i < imx; i++)
    {
        for (j = 0; j < imy; j++)
        {
            if (img[j * imx + i] > max_intensity)
            {
                max_intensity = img[j * imx + i];
            }
...
{
    if (x >= workx && x <= workx + 1024 && y >= worky && y <= worky + 1024)
        return 1;
    return 0;
}
...
void myComputeImage(agsKernel *krn, int workx, int worky, float dose, int kernelLevel)
{
    ...
    myfft(mskp, mskpf, FFT_FORWORD);
    myVmul(2 * imxy, (float *)mskpf, 1.0 / ((float)imxy));  //对应归一化
    myShift(mskpf, mskf);
    myClearFloat(img);
    myClearComplex(km);
        ...
        for (layer = 0; layer < knlvl; ++layer)
        {
            ...
            for (j = 0; j < ny; ++j)
                for (i = 0; i < nx; ++i)
                {
                    ...
                    kernelConvHead[index] = kernelHead[index] * kernelCTHead[index] - kernelHead[index + 1] * kernelCTHead[index + 1];     //r
                    kernelConvHead[index + 1] = kernelHead[index + 1] * kernelCTHead[index] + kernelHead[index] * kernelCTHead[index + 1]; //i
                }
...
maskfft(float *mask, float dose)
{
    ...
    //fftwf_execute(fwdplan);
    // kiss_fftnd(fwdplan, mskp, mskpf);
    myfft(mskp, mskpf, FFT_FORWORD);                                //低频在四个角
...
void convolveKernelCpx(kiss_fft_cpx *matrix, kiss_fft_cpx *result,
...
#define COMBO_VERSION
    ...
    // printf("%d, %d", kernel->nkx, kernel->nky);
    myClearComplex(km);
    // printf("**************\n");
    // printf("%d, %d, %f\n", kernel->nkx, kernel->nky, *(kernel->krn));
    // printf("**************\n");
    fprintf(stderr, "Now convolving with kernel %d weight %f\n", 0, kernel->scale[0]);
    myKernelMult(kernel->nkx, kernel->nky, kernel->krn, matrix, km);
    myShift(km, kmp);       //低频不在中间
    // kiss_fftnd(bwdplan, kmp, imgpk);
    myfft(kmp, imgpk, FFT_INVERSE);
...
void simulateImageOpt(float *mask, float *image, int kernelIdx, float dose,
                      int kernelLevel)
{
    agsKernel **kernels = optKernels[kernelIdx];
    img = image;
...
//binary version
void simulateImageB(float *mask, unsigned char *image, int kernelIdx, float dose,
                    int kernelLevel)
{
...
int getPvband(float *inner, float *outer)
{
