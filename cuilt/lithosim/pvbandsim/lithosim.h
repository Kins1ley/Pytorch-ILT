...
void read_Eslope(int x, int y, int dfom, int is_horiz_frag, fftwf_complex *E)
{
    ...
    if (is_horiz_frag)
    {
        index2 = (y + dfom * 1) * imx + x;
    }
    else
    {
        index2 = y * imx + x + dfom * 1;
    }
                        ...
                        int iteration_num, int workx, int worky, int ollx, int olly)
{
    ...
    for (i = 0; i < shapes; i++)
    {
        ...
        while (head != NULL)
        {
            ...
            x += 512 - ollx;
            y += 512 - olly;
            int tempwx = workx + 512;
            int tempwy = worky + 512;
            if (!is_in_tile(tempwx, tempwy, x, y))
            {
                head = head->next;
                fragnum++;
                continue;
            ...
            {
                intensity_lut = horizontal_intensity_lut;
                size_int_lut = size_lut_int_horiz;
                slope_lut = horizontal_slope_lut;
                size_slope_lut = size_lut_slope_horiz;
            ...
            {
                intensity_lut = vertical_intensity_lut;
                size_int_lut = size_lut_int_vert;
                slope_lut = vertical_slope_lut;
                size_slope_lut = size_lut_slope_vert;
    ...
    free(vertical_intensity_lut);
    free(horizontal_intensity_lut);
    free(vertical_slope_lut);
    free(horizontal_slope_lut);
}

#define LOOK(s, p) fprintf(stderr, "%s = %e %e %e %e %e\n", s, ((float *)p)[0], ((float *)p)[1], ((float *)p)[2], ((float *)p)[3], ((float *)p)[4])
#define DUMPC(fn, m)                   \
    fprintf(stderr, "dumpc %s\n", fn); \
    agsWriteFloatsHst(fn, (float *)(m), 2 * imxy)
#define DUMPF(fn, m)                   \
    fprintf(stderr, "dumpf %s\n", fn); \
    agsWriteFloatsHst(fn, (float *)(m), imxy)
#define DUMPN(fn, m, n)                \
    fprintf(stderr, "dumpn %s\n", fn); \
    agsWriteFloatsHst(fn, (float *)(m), n)
                    ...
                    int workx, int worky, int iteration_num, float dose, int flag_retarget_yes)
{
    ...
    for (i = 0; i < (NUM_EVALS_PER_SITE * numfrags); i++)
        cost_function_slope[i] = 0; /* Initialization */
    for (i = 0; i < (NUM_EVALS_PER_SITE * numfrags); i++)
        cost_function_intensity[i] = 0; /* Initialization */
    ...
    for (i = 0; i < shapes; i++)
    {
        ...
        while (head != NULL)
        {
            ...
            int ii;
            for (ii = 0; ii < NUM_EVALS_PER_SITE; ii++)
                fprintf(fdebug, "CF%d: %f ", ii, cost_function[costcnt++]);
            fprintf(fdebug, "\n");
            head = head->next;
        }
    }
...
#define D_WORK(i) (d_work + i * (2 * imxy))
        ...
        case CMD_FRAGMENT_LOAD:
            ...
            for (i = 0; i < NUM_EVALS_PER_SITE * num_total_frags; i++)
                cost_function[i] = 0;
            break;
