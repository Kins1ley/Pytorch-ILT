...
void read_layout(const char *fname)
{
    ...
    for (i = 0; i < MAXLVL; i++)
    {
        ...
        if (lvlcnt[i] > 0)
        {
            fprintf(stderr, "Level %s (%d) count = %d\n", agsUnintern(i), i, lvlcnt[i]);
            lvldata[i] = (rect *)malloc(lvlcnt[i] * sizeof(rect));
            lvlcnt[i] = 0;
        }
    }
