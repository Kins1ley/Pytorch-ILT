...
extern void read_layout(const char *fname);
...
public:
    ...
    {
        LITHO_KERNEL_FOCUS,
        LITHO_KERNEL_DEFOCUS,
        LITHO_KERNEL_FOCUS_CT,
        LITHO_KERNEL_DEFOCUS_CT,
        LITHO_KERNEL_FOCUS_PC,
        LITHO_KERNEL_DEFOCUS_PC
    } kernel_t;
    LithosimWrapper();
    ~LithosimWrapper();

    void writeMaskPng(char *fileName, float *mask, int xSize = 2048, int ySize = 2048);
    //void writeMaskPng(char* fileName_1, char* fileName_2, float* mask[2], int xSize = 2048, int ySize = 2048);
    ...
    void writeImageEpePng(char *fileName, float *image, float dose,
                          const char *inLayoutFile = NULL, int xSize = 2048, int ySize = 2048);
...
private:
    ...
    unsigned char *m_mask;
    unsigned char *m_mask_1;
    unsigned char *m_mask_2;
    float *m_image[NUM_PROCESS_CORNERS];
    float *m_subImage_1[NUM_PROCESS_CORNERS];
    float *m_subImage_2[NUM_PROCESS_CORNERS];
};
